...
public class ProbabilityManager : MonoBehaviour
{
    ...
    [SerializeField] private int copiesPerValue = 4; // cu�ntas copias de cada carta (1�11)
    ...
    /// - Qui�n roba (Player / Wizard)
    /// - Si los ojos del jugador est�n abiertos
    ...
    public int GetNextCard(Target target, bool playerEyesOpen, int currentTotal)
    {
        ...
        {
            Debug.LogWarning("Mazo vac�o, reconstruyendo deck...");
            BuildDeck();
        }

        // 1. Probabilidad de carta buena seg�n dificultad
        float probGood;
        ...
        // 2. Construir pools de cartas buenas y malas seg�n el total actual
        List<int> goodPool = GetGoodPool(currentTotal);
        List<int> badPool = GetBadPool(currentTotal);

        // Asegurarnos de que al menos tengamos alg�n pool con cartas
        if (goodPool.Count == 0 && badPool.Count == 0)
        {
            // Si por alguna raz�n no hay pools v�lidos, elegimos cualquiera del mazo
        ...
        {
            // Si el pool "ideal" est� vac�o, usamos el otro
        ...
        // 4. Quitamos ESA carta espec�fica del mazo
        ...
        {
            Debug.LogWarning("Carta elegida no encontrada en el mazo. Esto no deber�a pasar.");
        }
    ...
    private List<int> GetGoodPool(int total)
    {
        ...
        {
            // L�gica simple: buena = ayuda a acercarse a 21 sin matar
        ...
        // Si ninguna cumple ese criterio, aflojamos la condici�n:
        if (pool.Count == 0)
        {
            foreach (int card in remainingCards)
            {
